...
BtreeNode::BtreeNode(AttrType attrType, int attrLength,
                     ...
                     int pageSize) : attrLength(attrLength),
                                     attrType(attrType)
{
  keys = NULL;
  rids = NULL;
  ...
  while (((order) * (attrLength + sizeof(RID))) > ((unsigned int)pageSize - sizeof(numKeys) - 2 * sizeof(PageNum)))
    order--;
  char *pData = NULL;
  ...
  ph.GetPageNum(p);
  SetPageRID(RID(p, -1));
  keys = pData + sizeof(PF_PageHdr);
  rids = (RID *)(pData + attrLength * (order) + sizeof(PF_PageHdr));
  if (!newPage)
  {
